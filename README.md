...
Each organizer account has its own Durable Object, which stores the account's poll index and authentication records. Passwords are stored as PBKDF2-SHA-256 hashes with random salts and 310,000 iterations; session secrets are also hashed at rest. Registration and sign-in attempts are rate limited, and each account retains at most 10 active sessions. The browser keeps only the opaque session token in tab-scoped session storage. The local Express API follows the same model in its JSON data files.
